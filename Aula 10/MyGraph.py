...
class MyGraph:
    ...
    ## Hamiltonian
        #coleção p -> Uma string s com composição de k-mers igual a p (se existir)

    def check_if_valid_path(self, p): #p -> path
        #coleção p -> Uma string s com composição de k-mers igual a p (se existir)
        if p[0] not in self.graph.keys():# se o primeiro fragmento nao estiver nas keys do grafo
            return False#nao existe caminho por isso -> False
        for i in range(1,len(p)):
            #range(1,len(p))-> porque ja foi confirmado em cima que o p[0] esta nas keys do dicionario, sendo o primeiro do caminho
            if p[i] not in self.graph.keys() or p[i] not in self.graph[p[i-1]]:
                #EXEMPLO:
                #path = [’ACC−2’, ’CCA−8’, ’CAT−5’, ’ATG−3’]
                #se p[1] nao estiver nas keys do grafo OU p[1] nao estiver nos values de p[0] (self.graph[p[i-1]])
                return False#retorna falso porque nao e um caminho valido
        return True#se tudo se verificar e um caminho valido
        
    def check_if_hamiltonian_path(self, p):#p -> path
        '''Circuito Hamiltoniano é um caminho que passa por todos os nós do grafo exatamente uma vez'''
        if not self.check_if_valid_path(p): #verificar se e um caminho valido
            return False#se nao for nao se pode fazer o caminho hamiltonian
        to_visit = list(self.get_nodes())#ir buscar todos os nos, ou seja, keys
        if len(p) != len(to_visit): #se o caminho for diferente da lista de keys
            return False#nao existe caminho hamiltoniano
        for i in range(len(p)):#para cada fragmento em p
            if p[i] in to_visit: #se o fragmento estiver em p e na lista to_visit
                to_visit.remove(p[i])#remover o fragmento de p da lista
            else: #caso contrario
                return False#retornar falso, porque nao passa uma vez nos vertices(nos) todos do grafo
        if not to_visit: 
            #SE to_visit nao tiver NADA:
            return True#existe um caminho halmiltoniano
        else: #caso contrario
            return False#nao existe caminho hamiltoniano
    
    def search_hamiltonian_path(self):
        for ke in self.graph.keys():#para cafa fragmento da seq no grafo
    ...
    def search_hamiltonian_path_from_node(self, start):
        current = start #nó a processar
        visited = {start:0}#mantém estado dos nós/ arcos já explorados
        path = [start]#mantém caminho atual
        while len(path) < len(self.get_nodes()):#enquanto o path for menor que todos os fragmentos
            '''Caso em que nó é adicionado ao caminho'''
            nxt_index = visited[current]#vai aos values do no atual -> um numero
            if len(self.graph[current]) > nxt_index:#se o no atual tiver mais values que o visitado
                nxtnode = self.graph[current][nxt_index]#[no][numero] -> vai ao grafo a key e aos values ([no][0]) e tira o fragmento que esta na posicao indicada
                visited[current] += 1#adicionar 1 ao value do no atual no visitado
                if nxtnode not in path:#se o proximo no nao estiver no path(caminho)
                    path.append(nxtnode)#adicionar o proximo no ao caminho
                    visited[nxtnode] = 0 #value do proximo no = 0                    
                    current = nxtnode #no atual e atualizado     
            else: #se len(self.graph[current]) MENOR(<) nxt_index temos de fazer backtracking -> com repeticoes quer dizer que ja repetiu as vezes todas que podia, tendo de fazer backtracking
                if len(path) > 1: #temos de fazer backtracking -> ou seja eliminar o ultimo no em path
                    '''Backtracking: recuar para testar caminhos alternativos'''
                    rmvnode = path.pop()#elimina o ultimo elemento de path
                    del visited[rmvnode]#remover o ultimo no dos visitados
                    current = path[-1]#no atual passa a ser o ultimo do path
                else: 
                    return None
        return path#retornar o caminho

        #EXEMPLO:
        ''' res =[1,2,3,4,5]
            res.pop()
            print(res[-1]) -> IMPRIME: 4 '''

        #EXEMPLO:  nxtnode = self.graph[current][nxt_index]
        '''graph = {'TAC':['ACT','ACA'],'ATC':['TCA','TCG','TCC']}
        atual = 'TAC'
        visitado = {'TAC':0}
        nxt = visitado['TAC']
        if len(graph[atual])>nxt:
            nxtnode = graph[atual][nxt]
            print(nxtnode)'''

    # Eulerian
    
    def check_balanced_node(self, node):#verificar se o grau de entrada = grau de saida (para nos)
        return self.in_degree(node) == self.out_degree(node)
        
    def check_balanced_graph(self):#verificar se o grafo e balanceado
        for n in self.graph.keys():
            if not self.check_balanced_node(n): 
                return False
        return True
    
    def check_nearly_balanced_graph(self):#verificar se o grafo e semi balanceado
        res = None, None
        for n in self.graph.keys():
            indeg= self.in_degree(n)
            outdeg= self.out_degree(n)
            if indeg - outdeg == 1 and res[1] is None:#indegree > que outdegree
                res = res[0], n#vai colocar no primeiro
            elif indeg - outdeg == -1 and res[0] is None: #indegree < outdegree
                res = n, res[1]#vai colocar no segundo
            elif indeg == outdeg: 
                pass
            else: 
                return None, None 
    ...
    def eulerian_cycle(self):
        ...
        while edges_visit:#enquanto o edges_visit tiver elementos(??)
            ...
            if res != []:#se o res nao estiver vazio
                while pair[0] not in res:#se o primeiro arco nao estiver em res (ou seja, nao estiver 'coberto')
            ...
            if not res: #se o res for vazio
                res = cycle#vai juntar o primeiro ciclo
            else:#se nao vai juntar os ciclos
                pos = res.index(cycle[0])#vai ver o primeiro ciclo 
                for i in range(len(cycle)-1): 
                    res.insert(pos + i +1, cycle[i+1])
                    #vai inserir o novo ciclo a seguir ao anterior, por exemplo, i = 2, o novo ciclo vai ficar a seguir ao promeiro[0] ao i[2] e +1 para ficar na posicao 3
        return res                 
      
    def eulerian_path(self):
        '''Função para retornar caminho Euleriano (se existir) usando algoritmo anterior'''
        unb = self.check_nearly_balanced_graph()# res[0], key OU key, res[0] OU None, None
