...
class BWT:
    ...
    def build_bwt(self, text, buildsufarray = False):
        ...
        for i in range(len(text)):
            ls.append(text[i:]+text[:i])#construir as rotacoes ciclicas
        ls.sort()
        res =""
        for i in range(len(text)):
            res += ls[i][len(text)-1]
    ...
    def inverse_bwt(self):
        firstcol = self.get_first_col()#primeira coluna
        res = ""
        c = "$" 
        occ = 1 #primeira ocorrencia e o 1
        for i in range(len(self.bwt)):#ultima coluna
